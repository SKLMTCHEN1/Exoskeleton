...
extern void CopeSerial1Data(unsigned char ucData1);
...
#if EN_USART1_RX	
	USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);//开启相关中断（接受非空就中断）
...
//void USART1_IRQHandler(void)                	//串口1中断服务程序               4.important point need to check!
//{
//	u8 Res;
...
void USART1_Put_String(unsigned char *Str,unsigned char length)
{
	int i;
	for( i = 0; i<length;i++)
	{
